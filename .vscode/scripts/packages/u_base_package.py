...
class UGitPackage(UBasePackage):
    ...
    def check_installed(self, ctx: Context):
        ...
        if not os.path.exists(self.package_dir):
            return False
        with ctx.prefix(u_utils.change_dir_prefix(self.package_dir)):
            ...
            if not u_utils.question("Do you want to switch version?"):
    ...
    def install(self, ctx: Context):
        ...
        if os.path.exists(self.package_dir):
            ...
            if not u_utils.question("Do you want to remove the directory and re-install?"):
                raise UAbortedException
            with ctx.prefix(u_utils.change_dir_prefix(self.package_dir)):
                # Check if repo is dirty
                if not ctx.run(f"git diff-files --quiet", hide=True).ok:
...
class UArchivePackage(UBasePackage):
    ...
    def install(self, ctx: Context):
        ...
        u_utils.download_and_extract(url, self.package_dir, skip_first_subdir)
