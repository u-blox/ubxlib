...
LIB_NAME ?= lib$(NAME)
INCLUDE += $(UBXLIB_PATH)/common/error/api $(LIB_COMMON_PATH)/api
...
$(library_c): $(library_bin)
	...
	$(v)echo "/** Autogenerated file */" > $@
	$(v)echo "#include <stdint.h>" >> $@
	$(v)echo "const uint8_t __$(LIB_NAME)_blob[] = {" >> $@
	$(v)xxd -i < $(library_bin) >> $@
	$(v)echo "};" >> $@
	@echo "$(abspath $@)"
...
$(library_bin): $(library_descr_bin) $(library_code_final_bin)
	...
	$(v)cat $(library_descr_bin) $(library_code_final_bin) > $@
	@echo "$(abspath $@)"
...
$(library_code_sym): $(library_code_elf) $(library_code_bin)
	...
	$(v)echo "length = $(shell stat -L -c %s $(library_code_bin))" >> $@
