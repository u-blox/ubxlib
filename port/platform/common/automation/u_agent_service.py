...
# The maximum length of the ubxlib subdirectory
# name generated by _ubxlib_dir_path_create()
# Must be longer than
# AGENT_UBXLIB_PATH_CONTROLLER_NAME_MAX_LENGTH,
# see _ubxlib_dir_path_create() for details
AGENT_UBXLIB_SUBDIR_MAX_LENGTH = 16

# The working directory name to use within
# _ubxlib_dir_path
AGENT_WORKING_SUBDIR_NAME = "_work"
...
#########################################################
    ...
    # up to AGENT_UBXLIB_PATH_CONTROLLER_NAME_MAX_LENGTH
    # characters taken from the controller name.
    # If you're wondering why all this complexity,
    # it is to keep path lengths short.
    def _ubxlib_dir_path_create(self, url, branch, controller_name):
        '''Create a fully formed ubxlib directory path'''
        keep_going = True
        branch_rotate = 0
        branch_local = branch
        url_rotate = 0
        ...
        while keep_going:
            ...
            if controller_name:
                ...
                start = len(controller_name) - AGENT_UBXLIB_PATH_CONTROLLER_NAME_MAX_LENGTH
                if start < 0:
                    start = 0
                path += controller_name[start : len(controller_name)]
            length = AGENT_UBXLIB_SUBDIR_MAX_LENGTH - len(path)
            ...
            for ubxlib_dir_path in self._ubxlib_dir_paths:
                if ubxlib_dir_path["path"] == path:
                    if ((ubxlib_dir_path["url"] != url) or
                        (ubxlib_dir_path["branch"] != branch) or
                        (ubxlib_dir_path["controller_name"] != controller_name)):
            ...
            if path:
                ...
                if not reuse_existing:
                    ...
                    ubxlib_dir_path = {}
                    ubxlib_dir_path["path"] = path
                    ubxlib_dir_path["url"] = url
                    ubxlib_dir_path["branch"] = branch
                    ubxlib_dir_path["controller_name"] = controller_name
                    self._ubxlib_dir_paths.append(ubxlib_dir_path)
                    self._save_status()
            else:
    ...
    # Get the ubxlib path for the given url, branch
    # and controller name.
    def _ubxlib_dir_path_get(self, url, branch, controller_name):
        '''Return the ubxlib directory path'''
        path = ""
        for ubxlib_dir_path in self._ubxlib_dir_paths:
            if ubxlib_dir_path["url"] == url and \
               ubxlib_dir_path["branch"] == branch and \
               ubxlib_dir_path["controller_name"] == controller_name:
                path = ubxlib_dir_path["path"]
                break
        return path
    ...
    def _locker(self, controller_name, function_name=None):
        '''Check if controller_name is the current lockers'''
        is_locker = True
    ...
    def _save_status(self):
        ...
        status["ubxlib_dir_paths"] = self._ubxlib_dir_paths
        with open(AGENT_DYNAMIC_STATUS_FILE_NAME, "w") as file:
    ...
    def _restore_status(self):
        ...
        try:
            with open(AGENT_DYNAMIC_STATUS_FILE_NAME, "r") as file:
                try:
                    ...
                    if ("instances_supported" in status) and \
                       (status["instances_supported"] == self._instances_supported):
                        ...
                        # otherwise the activated may be out of date
                        self._instances_activated = status["instances_activated"]
                    if "ubxlib_dir_paths" in status:
                        for ubxlib_dir_path in status["ubxlib_dir_paths"]:
                            # Only keep ubxlib_dir_paths that exist
                            if ("path" in ubxlib_dir_path) and  \
                               os.path.exists(ubxlib_dir_path["path"]):
                                self._ubxlib_dir_paths.append(ubxlib_dir_path)
...
#########################################################
    ...
    # puts code into the path specified by _ubxlib_dir_path()
    # for an instance to be run on; we're talking replacing
    # both this script and the u_blah ones it includes here.
...
#########################################################
    ...
    # into a sub-directory defined by the method _ubxlib_dir_path()
    ...
    def exposed_check_out(self, url, branch, controller_name=None, clean=False):
        ...
        if self._locker(controller_name, "check_out"):
            ...
            if self._instances_activated:
                ...
                # controller names
                ubxlib_dir = self._ubxlib_dir_path_create(url, branch, controller_name)
                if ubxlib_dir:
                    if clean:
    ...
    # Note that runs are always clean because of the way
    # _ubxlib_dir_path works; it cannot guarantee not to re-use
    # an old directory.
            ...
            if self._instances_activated:
                ...
                if return_value == 0:
                    ...
                    if u_agent.start(hw_reset=hw_reset):
                        ...
                        ubxlib_dir = self._ubxlib_dir_path_get(url, branch, controller_name)
                        if ubxlib_dir:
                            # Create an empty working directory
                            working_dir = ubxlib_dir + os.sep + AGENT_WORKING_SUBDIR_NAME
                            if os.path.isdir(working_dir):
                                u_utils.deltree(working_dir, self._printer, PROMPT)
                            try:
    ...
    def exposed_result_path_get(self, url, branch, controller_name=None):
        ...
        path = self._ubxlib_dir_path_get(url, branch, controller_name)
        if path:
            path += os.sep + AGENT_WORKING_SUBDIR_NAME
        return path
    ...
    # sub-directories) to the given URL.  The credentials given at
    # agent start-up will be used.  The URL might, for instance, be
