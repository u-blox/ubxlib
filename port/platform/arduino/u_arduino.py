...
# A list of sub-strings we can use to order the include list,
# top-most at the start
INCLUDE_SORT_STRINGS = ["u_cfg_sw.h", "u_error_common.h", "u_cfg_", "u_port.h", "u_port_debug.h",
                        "u_port", "u_at_client.h", "module_type.h", "u_short_range.h", "u_network.h", "u_sock.h", "common" + os.sep]

# The comment at the top of the header file
HEADER_COMMENT = "/** This file is automatically generated by\n"   \
                 " * {0} to include all of the public\n"           \
                 " * headers of ubxlib.  The headers are sorted\n" \
                 " * during creation so that the order of\n"       \
                 " * inclusion is correct.  If you add a header\n" \
                 " * to ubxlib which needs to be sorted towards\n" \
                 " * the top (i.e. it has some new fundamental\n"  \
                 " * type in it that is required by other\n"       \
                 " * headers) then you may need to modify\n"       \
                 " * INCLUDE_SORT_STRINGS in {0} to make\n"        \
                 " * ubxlib compile for Arduino once more.\n"      \
...
# The header files have to be ordered correctly to work
def sort_include_files(include_files):
    '''Sort the includes files into an order which might work'''
    for sort_string in reversed(INCLUDE_SORT_STRINGS):
        include_files.sort(key=lambda x: sort_string in x)
    include_files.reverse()
...
def main(source_files, include_paths, platform_type, ubxlib_dir,
    ...
    if return_value >= 0:
        # Next, create a "ubxlib" header file, which is the best way to
        # introduce our stuff to the Arduino library manager
        print("Creating \"{}\"...".format(UBXLIB_HEADER_FILE))
        # Before we write the list of include files we must sort
        # them into an order that has a chance of working
        sort_include_files(include_files)
        return_value = u_arduino_common.create_header_file(UBXLIB_HEADER_FILE,
                                                           HEADER_COMMENT,
                                                           include_files)
