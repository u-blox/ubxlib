...
int32_t uGnssPwrOn(uDeviceHandle_t gnssHandle)
{
    ...
    if (gUGnssPrivateMutex != NULL) {
        ...
        if (pInstance != NULL) {
            ...
            if ((errorCode == 0) && (pInstance->transportType == U_GNSS_TRANSPORT_UBX_UART)) {
                errorCode = (int32_t) U_ERROR_COMMON_PLATFORM;
                ...
                // using uGnssPrivateSendReceiveUbxMessage() and
                // indicating that we don't care about the response.
                // But before we do that we send UBX-MON-MSGPP to
                // determine the number of message received by the
                // GNSS chip on the UART port and then we check it
                // again afterwards to be sure that our UBX-CFG-PRT
                // message really was received.
                if (uGnssPrivateSendReceiveUbxMessage(pInstance,
                    ...
                    if (uGnssPrivateSendReceiveUbxMessage(pInstance,
                        ...
                        errorCode = uGnssPrivateSendReceiveUbxMessage(pInstance,
                                                                      ...
                                                                      message, 20,
                                                                      NULL, 0);
                        // Skip any serial port purturbance at the far end
                        uPortTaskBlock(100);
    ...
    if (gUGnssPrivateMutex != NULL) {
        ...
        if (pInstance != NULL) {
            if (pInstance->transportType == U_GNSS_TRANSPORT_UBX_AT) {
                ...
                errorCode = (int32_t) U_ERROR_COMMON_PLATFORM;
                // Put the GNSS chip into low power mode with UBX-RXM-PMRE
                // The message does not have a response
                message[11] = 0x02; // Backup
                //lint -save -e569 Suppress loss of information: OK on all our compilers
                message[15] = 0xe4; // Wake-up on all sources
                //lint -restore
                if (uGnssPrivateSendOnlyUartUbxMessage(pInstance,
                                                       0x02, 0x41,
                                                       message,
                                                       sizeof(message)) == sizeof(message)) {
                    errorCode = (int32_t) U_ERROR_COMMON_SUCCESS;
